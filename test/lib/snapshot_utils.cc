...
#include "snapshot_utils.hh"


// Snapshot tests and their helpers
// \param func: function to be called back, in a seastar thread.
future<> do_with_some_data_in_thread(std::vector<sstring> cf_names, std::function<void (cql_test_env&)> func, bool create_mvs, shared_ptr<db::config> db_cfg_ptr, size_t num_keys) {
...
future<> take_snapshot(cql_test_env& e, sstring ks_name, sstring cf_name, sstring snapshot_name, db::snapshot_options opts) {
    try {
